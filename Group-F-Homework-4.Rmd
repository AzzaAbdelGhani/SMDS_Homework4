...
We can see that the estimated coefficients are equal, the difference between the two models is the dispersion parameter: in the Poisson one is set to 1, in the quasipoisson is set to 15. We can see that this mkae the standard errors change in the two models, and so the confidence intervals and the p-values.
=======
The highest numbers are in habitats `SWsoak` and `Disturbed`. Number of moths increases with the transect lenght, by a factor of $e^{0.55} =1.73$ for each one meter increase in transect lenght.
>>>>>>> 1fadfd0b7764c316a44fd0cc9a76518fe29acd87
